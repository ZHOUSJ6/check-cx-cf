...
import { getSanitizedErrorDetail } from "#/lib/error-handler";
...
function deriveBaseURL(endpoint: string): string {
  const pathWithoutQuery = endpoint.split("?")[0] ?? endpoint;
...
function parseModelDirective(model: string): {
  ...
  if (directiveMatch) {
    const baseModel = directiveMatch[1]?.trim() || trimmed;
    const effortKey = directiveMatch[2]?.toLowerCase();
    return {
      modelId: baseModel,
      reasoningEffort: effortKey ? REASONING_EFFORT_ALIASES[effortKey] : undefined,
    };
  }
